...
public class PersonalizationFactory
{
	 ...
	 *
	 * @param pers
	 *            object which contains the whole personalization
	 * @param writer
	 *            object which will be filled with the serialized personalization
	 */
	 ...
	 *
	 * @param reader
	 *            object which the personalization
	...
	public static Object unmarshal(String path) throws FileNotFoundException
	{
		...
		if (xmlFile.exists()) {
			BasicLogger.log("File at " + path + " found", LogLevel.TRACE, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.SYSTEM_TAG_ID));
		}
		else {
	...
	private static XStream getXStream()
	{
		...
		if (Activator.getContext() != null) {
			ServiceTracker<Converter, Converter> serviceTracker = new ServiceTracker<>(Activator.getContext(), Converter.class.getName(), null);
			serviceTracker.open();
			...
			BasicLogger.log(availableConverters.toString(), LogLevel.TRACE, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.SYSTEM_TAG_ID));
		}
		else {
			BasicLogger.log("Could not get the bundle context, no Converter services added to XStream", LogLevel.ERROR, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.SYSTEM_TAG_ID));
		}
