...
public class RiProtocolTest extends PersoSimTestCase {
	...
	public void setUp(){
		...
		protocol.setCardStateAccessor(testCardState);
	...
	public void testCalculateSectorIdentifier() throws Exception{
		//prepare the mock
		keyAgreement = KeyAgreement.getInstance("ECDH", Crypto.getCryptoProvider());
		messageDigest = new MessageDigest("foo") {
...
//		};
		...
		byte [] result = (byte[]) ReflectionHelper.invoke(RiProtocol.class, protocol, "calculateSectorIdentifier", 
				new Class<?>[] {PrivateKey.class, PublicKey.class, KeyAgreement.class, MessageDigest.class},
				privateKey, publicKey, keyAgreement, messageDigest
		);
	 ...
	 */
	@Test
	public void testCheckSectorPublicKeyHash() throws Exception {
...
//			}
//		};
		assertTrue((boolean)ReflectionHelper.invoke(RiProtocol.class, protocol, "checkSectorPublicKeyHash", 
				new Class<?>[] {ConstructedTlvDataObject.class, MessageDigest.class, byte[].class },
				sectorPublicKey, messageDigest, expectedResult.toByteArray())
		);
...
//		Deencapsulation.setField(keyObject, keypair);
	...
	public void testGeneralAuthenticate_NonAtTerminalType() throws Exception{
		...
		testCardState.putSecMechanism(TerminalAuthenticationMechanism.class, new TerminalAuthenticationMechanism(new byte[] {}, TerminalType.AT, null, null, null, null, null));
