...
public abstract class AbstractCommandProcessor extends Layer implements CardStateAccessor, StateMachine
{
	...
	{
		log("will now begin processing of ascending APDU", LogLevel.TRACE, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
		...
		try {
			...
			if (processingData.getResponseApdu() != null && PlatformUtil.is4xxxStatusWord(processingData.getResponseApdu().getStatusWord())) {
				log("APDU contents could not be processed by any protocol", LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
				...
				this.processingData.updateResponseAPDU(this, "No protocol was able to process the APDU contents", rApdu);
			}
	...
	public void powerOn()
	{
		...
		log("powerOn, remove all protocols from stack", LogLevel.TRACE, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
		setStackPointerToBottom();
		removeCurrentProtocolAndAboveFromStack();
		reset();

		log("powerOn, reset SecStatus", LogLevel.TRACE, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
	...
	// --------------------------------------------------------
	@Override
	public Collection<SecMechanism> getCurrentMechanisms(SecContext context, Collection<Class<? extends SecMechanism>> wantedMechanisms)
	{
	...
	{
		return (this.protocolStack.isEmpty() || (this.protocolStack.size() <= this.stackPointer));
	}
	...
	public void makeStackPointerCurrentlyActiveProtocol()
	{
		...
		log("Currently active protocol is now: " + getCurrentlyActiveProtocol().getProtocolName(), LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
		log("Currently active protocol is now: " + getCurrentlyActiveProtocol().getProtocolName(), LogLevel.INFO, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.PROTOCOL_TAG_ID));
	}
	...
	{
		log("Protocol chosen for processing is: " + getCurrentlyActiveProtocol().getProtocolName(), LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
		log("Current Processing Protocol is: " + getCurrentlyActiveProtocol().getProtocolName(), LogLevel.INFO, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.PROTOCOL_TAG_ID));
	 ...
	 *
	 * @return true if a ProtocolUpdate is found in processingData that
	 *         requests removal of the current protocol.
	 */
	public boolean isProtocolFinished()
	{
		LinkedList<UpdatePropagation> protocolUpdates = processingData.getUpdatePropagations(ProtocolUpdate.class);

		try {
			UpdatePropagation lastProtocolUpdate = protocolUpdates.getLast();

			if (lastProtocolUpdate instanceof ProtocolUpdate protocolUpdate) {
				return protocolUpdate.isFinished();
			}
			else {
				return false;
			}
	...
	{
		log("started cleaning up stack - will commence top down", LogLevel.TRACE, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
		for (int i = this.protocolStack.size() - 1; i >= this.stackPointer; i--) {
			log("removing protocol " + this.protocolStack.get(i).getProtocolName() + " from stack", LogLevel.TRACE, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
			this.protocolStack.remove(i);
		}
		log("finished cleaning up stack", LogLevel.TRACE, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
	}
	...
	public void addProtocolAtProtocolPointerToStack()
	{
		...
		log("protocol put to top of stack is " + protocol.getProtocolName(), LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
	...
	{
		Protocol protocol = protocols.get(protocolPointer);
		return protocol != null && protocol.isMoveToStackRequested();
	}
	...
	public void setStatusWordForUnsupportedCommand()
	{
		if (processingData.getResponseApdu() != null && PlatformUtil.is4xxxStatusWord(processingData.getResponseApdu().getStatusWord())) {
			...
			log("APDU not supported", LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
			ResponseApdu rApdu = new ResponseApdu(Iso7816.SW_6D00_INS_NOT_SUPPORTED);
			this.processingData.updateResponseAPDU(this, "No protocol was able to process the APDU", rApdu);
