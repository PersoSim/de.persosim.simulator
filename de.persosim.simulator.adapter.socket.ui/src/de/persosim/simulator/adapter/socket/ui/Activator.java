...
{
	public static final String PLUGIN_ID = "de.persosim.adapter.socket.ui";
	...
	{
		BasicLogger.log("START Activator Adapter Socket UI", LogLevel.TRACE, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.SYSTEM_TAG_ID));
		context = bundleContext;
		INSTANCE = this;
		PersoSimPreferenceManager.setPreferenceAccessorIfNotAvailable(new EclipsePreferenceAccessor());
		serviceTracker = new ServiceTracker<>(bundleContext, Simulator.class.getName(), new ServiceTrackerCustomizer<Simulator, Simulator>() {
		...
		BasicLogger.log("END Activator Adapter Socket UI", LogLevel.TRACE, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.SYSTEM_TAG_ID));
	}
	...
	public static void startVsmartcard()
	{
		if (vscSimulatorSocket == null || !vscSimulatorSocket.isRunning()) {
			vscSimulatorSocket = new SocketAdapter(Integer.parseInt(PersoSimPreferenceManager.getPreference(PreferenceConstants.VSMARTCARD_PORT, VSmartCardProtocol.DEFAULT_PORT + "")),
					new VSmartCardProtocol(Activator.INSTANCE));
