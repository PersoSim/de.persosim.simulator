...
public class PinProtocolTest extends PersoSimTestCase implements Tr03110 {
	...
	public void setUp() throws AccessDeniedException {
		...
		pinObject = new TestAuthObject(new AuthObjectIdentifier(ID_PIN), "111111".getBytes(), "PIN", 6, 6, 3,
				pinManagementCondition,
				new OrSecCondition(new PaceWithPasswordSecurityCondition("PIN"),
						new PaceWithPasswordSecurityCondition("PUK")),
				new PaceWithPasswordSecurityCondition("PUK"), new PaceWithPasswordRunningSecurityCondition("PIN"), 10);
		...
		taMechanismIs = new TerminalAuthenticationMechanism(new byte[] { 1, 2, 3 }, TerminalType.IS, new ArrayList<>(),
				new byte[] { 1, 2, 3 }, new byte[] { 1, 2, 3 }, "test", null);
		taMechanismAt = new TerminalAuthenticationMechanism(new byte[] { 1, 2, 3 }, TerminalType.AT, new ArrayList<>(),
				new byte[] { 1, 2, 3 }, new byte[] { 1, 2, 3 }, "test", null);
	...
	public void testProcessCommandVerifyPassword() {
		...
		processingData.updateCommandApdu(this, "select file APDU", CommandApduFactory.createCommandApdu(apduBytes));

		// call mut
		protocol.process(processingData);

		// check results
		assertEquals("Statusword", SW_63C3_COUNTER_IS_3, processingData.getResponseApdu().getStatusWord());
		assertEquals("RetryCounterValue", 3, pinObject.getRetryCounterCurrentValue());
	}

	/**
	 * Negative test case. Send verify apdu to the simulator but the PinObject is
	 * null
	 */
	@Test
	public void testProcessCommandVerifyPassword_PasswordObjectIsNull() {
		...
		// check results
		assertEquals("Statusword", SW_6984_REFERENCE_DATA_NOT_USABLE, processingData.getResponseApdu().getStatusWord());
	}

	/**
	 * Positive test case. Send changePin APDU to the simulator and receives a 9000
	 * if the PIN was successfully changed.
	 */
	@Test
	public void testProcessCommandChangePassword() {
		...
		secStatus.updateMechanisms(new SecStatusMechanismUpdatePropagation(SecContext.APPLICATION,
				new PaceMechanism(Pace.id_PACE_ECDH_GM_AES_CBC_CMAC_128, pinObject, null, null, null, null)));
		...
		// check results
		assertEquals("Statusword", SW_9000_NO_ERROR, processingData.getResponseApdu().getStatusWord());
		assertArrayEquals("Password", "222222".getBytes(), pinObject.getPassword());
	}

	/**
	 * Negative test case. Send changePin APDU to the simulator and receives a 6984
	 * because the object has no retry counter.
	 */
	@Test
	public void testProcessCommandChangePassword_NoRetryCounter() {
		// select Apdu
		ProcessingData processingData = new ProcessingData();
		...
		// check results
		assertEquals("Statusword", SW_6982_SECURITY_STATUS_NOT_SATISFIED,
				processingData.getResponseApdu().getStatusWord());
		assertArrayEquals("Password", "111111".getBytes(), pinObject.getPassword());
	}

	/**
	 * Negative test case. Send changePin APDU with no pin to the simulator (tlvData
	 * is empty) and receives 6A80.
	 */
	@Test
	public void testProcessCommandChangePassword_EmptyPassword() {
		...
		// check results
		assertEquals("Statusword", SW_6A80_WRONG_DATA, processingData.getResponseApdu().getStatusWord());
		assertArrayEquals("Password", "111111".getBytes(), pinObject.getPassword());
	}
	...
	public void testProcessCommandUnblockPassword_PasswordBlocked() {
		...
		secStatus.updateMechanisms(
				new SecStatusMechanismUpdatePropagation(SecContext.APPLICATION, authMechanismAtPinMgmt));
	...
	/**
	 * Positive test case. Send apdu to activate the PIN and receives a 9000.
	 *
	...
	/**
	 * Negative test case. Send apdu to deactivate the Pin but Pin management rights
	 * from TA are required to perform the deactivate.
	 */
	@Test
	...
	/**
	 * Positive test case. Send apdu to unblock PIN, reset retry counter and
	 * receives 9000
	 */
	@Test
	public void testProcessCommandUnblockPassword_PasswordBlocked_ResetRetryCounterGreaterThanZero() {
		// prepare the environment
		pinObject.setRetryCounterCurrentValue(0);
		int resetRetryCounter = 1;
		pinObject.setResetRetryCounterCurrentValue(resetRetryCounter);
		secStatus.updateMechanisms(new SecStatusMechanismUpdatePropagation(SecContext.APPLICATION, taMechanismAt));
		secStatus.updateMechanisms(
				new SecStatusMechanismUpdatePropagation(SecContext.APPLICATION, authMechanismAtPinMgmt));
		...
		// check results
		assertEquals("Statusword is 9000", SW_9000_NO_ERROR, processingData.getResponseApdu().getStatusWord());
		assertEquals("ResetRetryCounterValue", (resetRetryCounter - 1), pinObject.getResetRetryCounterCurrentValue());
		pinObject.setResetRetryCounterCurrentValue(10);
	}

	/**
	 * Negative test case. Send apdu to unblock PIN, reset retry counter, but reset
	 * retry counter equals already 0
	 */
	@Test
	public void testProcessCommandUnblockPassword_PasswordBlocked_ResetRetryCounterZero() {
		// prepare the environment
		int resetRetryCounter = 0;
		pinObject.setRetryCounterCurrentValue(0);
		pinObject.setResetRetryCounterCurrentValue(resetRetryCounter);
		secStatus.updateMechanisms(new SecStatusMechanismUpdatePropagation(SecContext.APPLICATION, taMechanismAt));
		secStatus.updateMechanisms(
		...
		// check results
		assertEquals("Statusword", SW_6900_COMMAND_NOT_ALLOWED, processingData.getResponseApdu().getStatusWord());
		assertEquals("ResetRetryCounterValue", resetRetryCounter, pinObject.getResetRetryCounterCurrentValue());
		pinObject.setResetRetryCounterCurrentValue(10);
	}


	@Test
	public void testProcessCommandChangePassword_ResetRetryCounterZero() {
		// prepare the environment
		int resetRetryCounter = 0;
		pinObject.setResetRetryCounterCurrentValue(resetRetryCounter);
		secStatus.updateMechanisms(new SecStatusMechanismUpdatePropagation(SecContext.APPLICATION,
