...
public class ProcessingData implements Iso7816, InfoSource
{
	...
	public void updateProcessingState(InfoSource source, String message, ProcessingStateDelta... update)
	{
		...
		log(source, "Update processing state with " + update.length + " deltas.", LogLevel.TRACE, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
		log(source, "Update message\n" + message, LogLevel.TRACE, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
		for (ProcessingStateDelta curStateDelta : update) {
			if (curStateDelta != null && curStateDelta.getNrOfModifications() > 0) {
				...
				// log modifications accordingly
				log(source, curStateDelta.toString(), LogLevel.TRACE, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
				...
				if (curStateDelta.getCommandApdu() != null) {
					...
					log(source, "Command APDU updated\n" + commandApdu, LogLevel.TRACE, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
				}

				// update response APDU if present
				if (curStateDelta.getResponseApdu() != null) {
					this.responseApdu = curStateDelta.getResponseApdu();
					log(source, "Response APDU updated\n" + responseApdu + "\nreason is: " + message, LogLevel.TRACE, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
				}
				...
				if (curStateDelta.getUpdatePropagations() != null) {
					...
					for (Class<? extends UpdatePropagation> curKey : newPropagations.keySet()) {
						...
						if (curKey.isInstance(curNewProp)) {
							...
							log(source, "Skipping one UpdatePropagation, as type does not match key", LogLevel.WARN, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
						}
					}
				}
	 ...
	 */
	public LinkedList<UpdatePropagation> getUpdatePropagations(Class<? extends UpdatePropagation> key)
	{
	 ...
	 */
	public void addUpdatePropagation(InfoSource source, String message, UpdatePropagation updatePropagation)
	{
