...
public abstract class AbstractPaceProtocol extends AbstractProtocolStateMachine implements Pace, TlvConstants
{
	...
	/**
	 * @param bytes
	 *            the OID given in the SET AT command
		...
		try {
			...
			try {
				...
				if (paceOid == null) {
					...
					log("OID '" + HexString.encode(oidRaw) + "' does not match current protocol with prefix " + prefixOid, LogLevel.DEBUG,
			...
			if (pwdCandidate instanceof PasswordAuthObject) {
				...
				log("selected password is: " + getPasswordName(), LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
			}
			else {
			...
			processingData.addUpdatePropagation(this, protocolName + " started with " + pacePassword.getPasswordName(),
					new SecStatusMechanismUpdatePropagation(SecContext.APPLICATION, paceUsedPasswordMechanism));
			...
			log(logString, LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
			...
			log("common secret is: " + HexString.encode(commonSecret), LogLevel.TRACE, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));

			byte[] keyMaterialForEncryptionOfNonce = kdf.derivePI(commonSecret);

			log("computed raw key material of byte length " + keyMaterialForEncryptionOfNonce.length + " is: " + HexString.encode(keyMaterialForEncryptionOfNonce), LogLevel.TRACE,
					new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));

			this.secretKeySpecNonce = this.cryptoSupport.generateSecretKeySpecCipher(keyMaterialForEncryptionOfNonce);

			log("computed " + paceOid.getSymmetricCipherAlgorithmName() + " key material: " + HexString.encode(keyMaterialForEncryptionOfNonce), LogLevel.DEBUG,
					new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
	 ...
	 */
	public static boolean checkPasswordAndAccessRights(CertificateHolderAuthorizationTemplate chat, PasswordAuthObject password)
	{
		switch (chat.getTerminalType()) {
			case AT:
				if (password.getPasswordIdentifier() == ID_PIN
						|| (password.getPasswordIdentifier() == ID_CAN && chat.getRelativeAuthorization().getAuthorization().getBit(Tr03110Utils.ACCESS_RIGHTS_AT_CAN_ALLOWED_BIT))) {
					return true;
				}
				break;
			case IS:
				if (password.getPasswordIdentifier() == ID_CAN || password.getPasswordIdentifier() == ID_MRZ) {
					return true;
				}
				break;
			case ST:
				if (password.getPasswordIdentifier() == ID_CAN || password.getPasswordIdentifier() == ID_PUK || password.getPasswordIdentifier() == ID_PIN) {
					return true;
				}
				break;
		}
	...
	public void processCommandGetNonce()
	{
		...
		log("key length k in Bytes is " + keySizeInBytes + ", block size in Bytes is " + blockSizeInBytes
				+ " --> nonce s must be of smallest length l in Bytes, l being a multiple of the block size, such that l<=k", LogLevel.TRACE,
				new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));

		this.piccsPlainNonceS = new byte[nonceSizeInBytes];
		this.secureRandom.nextBytes(this.piccsPlainNonceS);

		log("new (plain) nonce s of byte length " + this.piccsPlainNonceS.length + " is " + HexString.encode(this.piccsPlainNonceS), LogLevel.TRACE,
				new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));

		encryptedNonce = this.cryptoSupport.encryptWithIvZero(this.piccsPlainNonceS, this.secretKeySpecNonce);

		log("(encryted) nonce z = E_KPi(s) is " + HexString.encode(encryptedNonce), LogLevel.TRACE, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));

		primitive80 = new PrimitiveTlvDataObject(TAG_80, encryptedNonce);
		log("primitive tag 80 is: " + primitive80, LogLevel.TRACE, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
	...
	public void processCommandMapNonce()
	{
		...
		log("mapping data received from PCD is expected to contain " + mapping.getMeaningOfMappingData(), LogLevel.DEBUG,
				new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
		log("unchecked mapping data content of " + mappingDataFromPcd.length + " bytes length is: " + HexString.encode(mappingDataFromPcd), LogLevel.DEBUG,
				new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
		log("nonce s generated by PICC during processing of GetNonce command is " + HexString.encode(piccsPlainNonceS), LogLevel.TRACE,
				new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));

		byte[] mappingResponse;

		try {
			log("about to perform " + mapping.getMappingName(), LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
		...
		log("PICC's ephemeral public  mapped " + keyAgreementName + " key is " + new TlvDataObjectContainer(ephemeralKeyPairPicc.getPublic().getEncoded()), LogLevel.TRACE,
				new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
		log("PICC's ephemeral private mapped " + keyAgreementName + " key is " + new TlvDataObjectContainer(ephemeralKeyPairPicc.getPrivate().getEncoded()), LogLevel.TRACE,
				new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
	...
	public void processCommandPerformKeyAgreement()
	{
		...
		log("PCD's public raw key of " + rawKeyPlain.length + " bytes length is: " + HexString.encode(rawKeyPlain), LogLevel.TRACE,
				new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
		...
		try {
			...
			log("PCD's  ephemeral public  mapped " + paceDomainParametersMapped.getKeyAgreementAlgorithm() + " key is " + new TlvDataObjectContainer(ephemeralPublicKeyPcd.getEncoded()),
					LogLevel.TRACE, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
		}
		catch (IllegalArgumentException e) {
			logException(e.getMessage(), e, LogLevel.ERROR, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
			ResponseApdu resp = new ResponseApdu(Iso7816.SW_6A80_WRONG_DATA);
		...
		log("bare response data of byte length " + ephemeralPublicKeyComponentPicc.length + " is " + HexString.encode(ephemeralPublicKeyComponentPicc), LogLevel.DEBUG,
				new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
	...
	public void processCommandMutualAuthenticate()
	{
		...
		try {
			...
			log("picc token raw data " + piccTokenInput, LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
			log("pcd  token raw data " + pcdTokenInput, LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
			...
			try {
				...
				log("shared secret of byte length " + sharedSecret.length + " resulting from " + paceOid.getKeyAgreementName() + " key agreement is " + HexString.encode(sharedSecret), LogLevel.DEBUG,
						new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
				...
				log("final " + secretKeySpecENC.getAlgorithm() + " symmetric key material ENC is " + HexString.encode(secretKeySpecENC.getEncoded()), LogLevel.DEBUG,
						new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
				log("final " + secretKeySpecMAC.getAlgorithm() + " symmetric key material MAC is " + HexString.encode(secretKeySpecMAC.getEncoded()), LogLevel.DEBUG,
						new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
			}
			...
			log("picc token data is: " + HexString.encode(piccToken), LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));

			byte[] pcdToken = Arrays.copyOf(this.cryptoSupport.macAuthenticationToken(pcdTokenInput.toByteArray(), this.secretKeySpecMAC), 8);
			log("pcd  token data is: " + HexString.encode(pcdToken), LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));

			log("expected pcd token data is: " + HexString.encode(pcdToken), LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
			log("received pcd token data is: " + HexString.encode(pcdTokenReceivedFromPCD), LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
			...
			if (Arrays.equals(pcdToken, pcdTokenReceivedFromPCD)) {
				log("Token received from PCD matches expected one", LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));

				if (isValidPasswordType()) {
					if (pacePassword instanceof PasswordAuthObjectWithRetryCounter) {
						ResponseData pinResponse = getMutualAuthenticatePinManagementResponseCESuccessful((PasswordAuthObjectWithRetryCounter) pacePassword, cardState);
				...
				// PACE failed
				log("Token received from PCD does NOT match expected one", LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
				if (pacePassword.getPasswordIdentifier() == Pace.ID_PIN) {
					ResponseData pinResponse = getMutualAuthenticatePinManagementResponseCEFailed((PasswordAuthObjectWithRetryCounter) pacePassword);
					sw = pinResponse.getStatusWord();
					note = pinResponse.getResponse();
				}
				else {
	...
	protected ResponseApdu doSmDataProviderHandling(short sw, ConstructedTlvDataObject responseContent)
	{
		...
		PaceMechanism paceMechanism = new PaceMechanism(paceOid, pacePassword, unmapped, paceDomainParametersMapped.comp(ephemeralKeyPairPicc.getPublic()),
				paceDomainParametersMapped.comp(ephemeralPublicKeyPcd), terminalTypeOid);
	...
	protected void addCars(ConstructedTlvDataObject constructed7c)
	{
		...
		if (trustPoint != null) {
			if (trustPoint.getCurrentCertificate() != null && trustPoint.getCurrentCertificate().getCertificateHolderReference() instanceof PublicKeyReference) {
				constructed7c.addTlvDataObject(new PrimitiveTlvDataObject(TAG_87, trustPoint.getCurrentCertificate().getCertificateHolderReference().getBytes()));
				if (trustPoint.getPreviousCertificate() != null && trustPoint.getPreviousCertificate().getCertificateHolderReference() instanceof PublicKeyReference) {
					constructed7c.addTlvDataObject(new PrimitiveTlvDataObject(TAG_88, trustPoint.getPreviousCertificate().getCertificateHolderReference().getBytes()));
				}
			}
		}
	}
	...
	public static ResponseData getMutualAuthenticatePinManagementResponsePaceFailed(PasswordAuthObjectWithRetryCounter pacePasswordPin, String protocolName)
	{
		...
		log(protocolName + " with PIN has failed - PIN retry counter will be decremented, current value is: " + pinRetryCounter, LogLevel.DEBUG,
				new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
		short sw;
		String note;
		...
		log(note, LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
		return new ResponseData(sw, note);
	}
			...
			if (pinRetryCounter != pinRetryCounterDefault) {
				if (pinRetryCounter == 1) {
					if (isPinTemporarilyResumed(cardState, protocolName)) {
						...
						sw = Iso7816.SW_9000_NO_ERROR;
						note = "MutualAuthenticate processed successfully with password PIN after CAN - PIN retry counter has been reset from: " + pinRetryCounter + " to: "
								+ pacePasswordPin.getRetryCounterCurrentValue();
					}
					else {
					...
					sw = Iso7816.SW_9000_NO_ERROR;
					note = "MutualAuthenticate processed successfully with password PIN - PIN retry counter has been reset from: " + pinRetryCounter + " to: "
							+ pacePasswordPin.getRetryCounterCurrentValue();
				}
			}
			else {
	...
	public static boolean isPinTemporarilyResumed(CardStateAccessor cardState, String protocolName)
	{
		...
		if (!currentMechanisms.isEmpty()) {
			...
			log("last successful " + protocolName + " run used " + getPasswordName(previouslyUsedPasswordIdentifier) + " as password with value " + HexString.encode(previouslyUsedPwd.getPassword()),
					LogLevel.DEBUG, new LogTag(BasicLogger.LOG_TAG_TAG_ID, PersoSimLogTags.COMMAND_PROCESSOR_TAG_ID));
	...
	public Collection<TlvDataObject> getSecInfos(SecInfoPublicity publicity, MasterFile mf)
	{
		...
		Collection<CardObject> domainParameterCardObjects = mf.findChildren(new DomainParameterSetIdentifier(), paceOidIdentifier);
	 ...
	 *
	 * @param secInfos
	 *            HashSet with all security infos
	 * @param paceOidIdentifier
	 *            the pace oid identifier
	 * @param domainParameterCardObjects
	 *            collection of card objects which matches the paceOidIdentifier
	 */
	public void createDomainParameterInfo(HashSet<TlvDataObject> secInfos, OidIdentifier paceOidIdentifier, Collection<CardObject> domainParameterCardObjects)
	{
