...
public class ChangeablePasswordAuthObject extends PasswordAuthObject {
	...
	protected boolean resetRetryCounterAvailable = true;
	...
	public ChangeablePasswordAuthObject(AuthObjectIdentifier identifier, byte[] password, String passwordName,
		...
		this(identifier, password, passwordName, minLengthOfPasswordInBytes, maxLengthOfPasswordInBytes,
				pinManagementCondition, changePinCondition, -1);
	}

	public ChangeablePasswordAuthObject(AuthObjectIdentifier identifier, byte[] password, String passwordName,
			int minLengthOfPasswordInBytes, int maxLengthOfPasswordInBytes, SecCondition pinManagementCondition,
			SecCondition changePinCondition, int maxResetRetryCounterValue) {

		super(identifier, password, passwordName);

		if (minLengthOfPasswordInBytes < 0) {
			throw new IllegalArgumentException(
					"minimum required length for " + passwordName + " in bytes must be >= 0");
		}
		if (maxLengthOfPasswordInBytes < minLengthOfPasswordInBytes) {
			throw new IllegalArgumentException(
					"maximum required length for " + passwordName + " in bytes must be >= minimum required length");
		}

		if (password.length < minLengthOfPasswordInBytes) {
			throw new IllegalArgumentException(passwordName + " must be at least " + minLengthOfPasswordInBytes
					+ " bytes long but is only " + password.length + " bytes long");
		}
		if (password.length > maxLengthOfPasswordInBytes) {
			throw new IllegalArgumentException(passwordName + " must be at most " + maxLengthOfPasswordInBytes
					+ " bytes long but is " + password.length + " bytes long");
		}
		...
		if (maxResetRetryCounterValue <= -1) {
			resetRetryCounterAvailable = false;
		}
		resetRetryCounterCurrentValue = maxResetRetryCounterValue;
	}

	public void setPassword(byte[] newPassword) throws AccessDeniedException {
		if (newPassword == null) {
			throw new IllegalArgumentException("new " + passwordName + " must not be null");
		}
		if (!getLifeCycleState().equals(Iso7816LifeCycleState.OPERATIONAL_ACTIVATED)
				&& !getLifeCycleState().isPersonalizationPhase()) {
			throw new IllegalStateException(passwordName + " must be in state operational activated");
		}

		if (!getLifeCycleState().isPersonalizationPhase()) {
			if (newPassword.length < minLengthOfPasswordInBytes) {
				throw new IllegalArgumentException("new " + passwordName + " must be at least "
						+ minLengthOfPasswordInBytes + " bytes long but is only " + newPassword.length + " bytes long");
			}
			if (newPassword.length > maxLengthOfPasswordInBytes) {
				throw new IllegalArgumentException("new " + passwordName + " must be at most "
						+ maxLengthOfPasswordInBytes + " bytes long but is " + newPassword.length + " bytes long");
			}
	...
	public void updateLifeCycleState(Iso7816LifeCycleState state) throws AccessDeniedException {
		if (securityStatus == null || !state.isOperational()
	...
	public void decrementResetRetryCounter() {
	...
	public boolean isResetRetryCounterAvailable() {
		return resetRetryCounterAvailable;
	}

	public int getResetRetryCounterCurrentValue() {
